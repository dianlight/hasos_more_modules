...
| `zzstd.ko` | ZFS Zstandard compression – from openzfs/zfs | ⚠️ Not available on `rpi3_64`, `rpi4_64`, `rpi5_64`, `yellow`: ZFS (CDDL) is license-incompatible with GPL-only kernel symbols exposed by Raspberry Pi kernels (kernel_neon_begin/end, bpf_trace_run*, trace_event_*). |
